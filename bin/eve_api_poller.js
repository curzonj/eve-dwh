...
function limitClient(client, endpoint, data) {
    return throttle.add(function() {
        debug('eve:xml', _.assign({ endpoint: endpoint }, data))
    ...
    return sql("eve_api_keys").then(function(rows) {
        return bluebird.each(rows, function(key_row) {
            ...
            if (key_row.is_corporate) {
                return bluebird.each(sql("managed_corps").where({ key_id: key_row.key_id }).pluck('corporation_id'),
                function(corp_id) {
                    return bluebird.resolve(fn(client, null, corp_id)).then(function() {
                        logfmt.log({ import: name, key_id: key_row.key_id, corporation_id: corp_id, at: "finished", fn: "eachApiKeyTarget" })
                    })
                }).then(function() {
                    logfmt.log({ import: name, key_id: key_row.key_id, at: "finished", fn: "eachApiKeyTarget" })
                })
            
            } else {
                return bluebird.each(sql("managed_characters").where({ key_id: key_row.key_id }).pluck('character_id'),
                function(char_id) {
                    return bluebird.resolve(fn(client, char_id)).then(function() {
                        logfmt.log({ import: name, key_id: key_row.key_id, character_id: char_id, at: "finished", fn: "eachApiKeyTarget" })
                    })
                }).then(function() {
                    logfmt.log({ import:name, key_id: key_row.key_id, at: "finished", fn: "eachApiKeyTarget" })
                })
...
lib.cronTask(1800, function() {
    return bluebird.try(function() {
        return eachApiKeyTarget('WalletTransactions', function(client, char_id, corp_id) {
            return bluebird.try(function() {
                if (char_id === null) {
                    return limitClient(client, 'corp:WalletTransactions', { accountKey: 1000 })
                } else {
                    return limitClient(client, 'char:WalletTransactions', { characterID: char_id })
                }
            }).then(function(data) {
                return bluebird.map(_.values(data.transactions),
                function(t) {
                    var client_id = parseInt(t.clientID)
                    lib.debug('import', { at: 'transaction', data: JSON.stringify(t) })
                ...
                if (char_id === null) {
                    return limitClient(client, 'corp:WalletJournal', { accountKey: 1000 })
                } else {
                    return limitClient(client, 'char:WalletJournal', { characterID: char_id })
                }
            }).then(function(data) {
                return bluebird.map(_.values(data.transactions),
                function(t) {
                    lib.debug('import', { at: 'journal', data: JSON.stringify(t) })
...
lib.cronTask(21600, function() {
    return bluebird.try(function() {
        ...
        return eachApiKeyTarget('MarketOrders', function(client, char_id, corp_id) {
            return bluebird.try(function() {
                if (char_id === null) { 
                    return limitClient(client, 'corp:MarketOrders', { corporationID: corp_id })
                } else {
                    return limitClient(client, 'char:MarketOrders', { characterID: char_id })
                }
            }).then(function(data) {
                return bluebird.map(_.values(data.orders),
                function(t) {
                    lib.debug('import', { at: 'order', data: JSON.stringify(t) })
                    ...
                    return sql.raw(sql("character_order_details").insert({
                            ...
                            region_id: sql("staStations").select("regionID").where({ stationID: station_id })
...
lib.cronTask(7200, function() {
    return bluebird.try(function() {
        ...
        return eachApiKeyTarget('AssetList', function(client, char_id, corp_id) {
            return bluebird.try(function() {
                if (char_id === null) { 
                    return limitClient(client, 'corp:AssetList', { corporationID: corp_id })
                } else {
                    return limitClient(client, 'char:AssetList', { characterID: char_id })
                }
            ...
            return bluebird.each(_.keys(virtual), function(pair) {
                ...
                pair = pair.split(',')
                var station_id = parseInt(pair[0]), type_id = parseInt(pair[1])

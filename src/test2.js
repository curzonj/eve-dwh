...
var client = new neow.EveClient({
  keyID: process.env.CHAR_KEYID,
  vCode: process.env.CHAR_VCODE,
}, null, neowDiskCache)
...
client.fetch('char:PlanetaryColonies', { characterID: 92453716 })
.then(function(colonies) {
  console.log(JSON.stringify(colonies, null, 2))
  ...
  return bluebird.each(_.values(colonies.colonies), planet => {
    return bluebird.try(function() {
      ...
      return rawEveResponse(function() {
        ...
        return client.fetch('char:PlanetaryRoutes', {
          characterID: planet.ownerID,
          planetID: planet.planetID,
      ...
      return bluebird.each(_.values(result), function(value) {
        if (value.schematicID > 0)
          return lib.sql('planetSchematicsTypeMap').where({

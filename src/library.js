...
function buildEveClient(key_id, vcode) {
    return new neow.EveClient({ keyID: key_id, vCode: vcode }, null, neowCache)
}
...
_.assign(exports, {
    ...
    cronTask: function(interval, fn) {
        function scheduleNext() {
            ...
            return fn().then(function(nextRun) {
    ...
    findInterestingMarketGroups: function() {
        ...
        function getSubGroups(id) {
            return sql("invMarketGroups").where({parentGroupID: id}).select("hasTypes", "marketGroupID").
            then(function(rows) {
        ...
        return bluebird.all([
            ...
            return bluebird.mapSeries(regions, function(row) {
                return bluebird.map(types, function(type_id) {
                    ...
                    return sql.raw(
                        sql("market_polling").insert({ type_id: type_id, region_id: row.regionID }).toString() +
    ...
    pollingLoop: function(fn) {
        function _innerLoop() {
            return fn().catch(function(e) {
                ...
                logfmt.namespace({ fn: "pollingLoop" }).error(e)
            }).then(function(sleep) {
                var delay = 1
                if (sleep)

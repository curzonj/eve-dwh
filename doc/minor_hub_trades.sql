select * from (
  ...
  select
  (select "typeName" from "invTypes" where "typeID" = type_id) type_name,
  (select "stationName" from "staStations" where "stationID" = hub.station_id) station_name,
  round(((hub.sell_price_wavg_sold - jita.sell_price_min) * est_market_share(3, hub.daily_sell_orders::numeric, hub.daily_sell_isking::numeric, hub.daily_sell_units::numeric)), 0) AS funny,
  hub.avg_orders,
  hub.daily_sell_units,
  hub.daily_sell_isking,
  jita.sell_price_min jita_sell,
  hub.sell_price_wavg_sold hub_sell,
  round((hub.sell_price_wavg_sold - jita.sell_price_min) / jita.sell_price_min, 2) AS hub_margin
...
order by station_name asc, funny desc;

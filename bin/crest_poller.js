...
_.times(orderConcurrency, function(n) {
    lib.pollingLoop(function market_orders() {
        const logger = logfmt.namespace({ fn: "crest_poller.js#market_orders", loop: uuidGen.v1(), worker: n })
        const timer = logger.time()

        return sql.transaction(function(trx) { return trx.raw("update market_polling a0 set order_polling_started_at = now() from (select type_id, region_id from market_polling where orders_next_polling_at < now() AND order_polling_started_at < (now() - interval '30 seconds') order by orders_polling_interval asc limit ? for update skip locked) as a1 where a0.type_id = a1.type_id and a0.region_id = a1.region_id returning a0.type_id, a0.region_id, a0.orders_polling_override", [ 10 ]).then(function(res) { return res.rows /* force knex to execute the query */ }) }).
        then(function(rows) {
            logger.log({ at: "start", rows_available: rows.length })
                ...
                return bluebird.map(rows, function(row) {
                    return importSingleOrderType(row.type_id, row.region_id).then(function() {
                        if (row.orders_polling_override !== null)
                            pubsub.emit('my_order_polling', { region_id: row.region_id, type_id: row.type_id })
                    })
                }).then(function() {
                    return false
                })
...
lib.pollingLoop(function() {
    const logger = logfmt.namespace({ fn: "crest_poller.js#order_histories", loop: uuidGen.v1() })
    const timer = logger.time()

    return sql.transaction(function(trx) {
        return trx.raw(sql("market_polling").
            whereRaw("history_next_polling_at < current_timestamp").orWhereNull("history_next_polling_at").
            orderBy("history_polling_interval", "asc").limit(historyConcurrency).
            toString() + " FOR UPDATE SKIP LOCKED"
        ).then(function(result) {
            logger.log({ at: "start", rows_available: result.rowCount })
                ...
                return bluebird.map(result.rows, function(row) {
                    return historyThrottle.add(function() {
                        return importSingleHistory(row.type_id, row.region_id).then(function() {
                            ...
                            where({ type_id: row.type_id, region_id: row.region_id }).
...
function eveRequest(url) {
    return bluebird.try(function() {
        return eveThrottle.add(function() {
            ...
            debug('eve:crest', { url: url, at: "start" })
            ...
            return bluebird.resolve(rp({
                ...
                debug('eve:crest', { at: "finished", url: url, elapsed: timer.end() })
            }).catch(rp.errors.RequestError, function(e) {
                logfmt.log({ fn: 'eveRequest', at: "error", url: url, message: e.message, elapsed: timer.end() })
                ...
                logfmt.log({ fn: 'eveRequest', at: "error", url: url, status_code: e.statusCode, message: message, elapsed: timer.end() })
                stats.meter('crest_errors').mark()
        ...
        return sql.transaction(function(trx) {
            return trx("market_polling").where({ type_id: type_id, region_id: region_id }).forUpdate().then(function() {
                ...
                function storeSold(station_id, side, units, price) {
                    change_stats_by_station[station_id][side+'_sold'].push({ units: units, price: price })
                }
                    ...
                    if (row === undefined) {
                        ...
                        return bluebird.try(function() {
                            return trx("market_orders").insert({
                                ...
                                region_id: trx("staStations").select("regionID").where({ stationID: o.location.id })
                            })
                        }).then(function() {
                ...
                return bluebird.all([
                    ...
                    if (stale_rows.length > 0) {
                        const still_active = _.filter(orders, (o)=>{ return _.includes(existing, o.id) } )
                        ...
                        return bluebird.each(stale_rows, function(row) {
                            ...
                            if (!canceled) {
                                ...
                                debug('math:canceled', { at: "canceled", order_id: row.id, details: JSON.stringify(row), criteria: JSON.stringify(canceled_criteria) })
                            }
                            ...
                            return bluebird.try(function() {
                                ...
                                return trx.raw(sql("historical_orders").insert(row).
                                toString() + " ON CONFLICT (id, first_observed_at) DO NOTHING")
                            })
                        }).then(function() {
                            // delete old orders
                            const stale_ids = _.map(stale_rows, 'id')
                            return trx("market_orders").whereIn('id', stale_ids).delete()
                        })
                    }
                }).then(function() {
                    var stationOrders = {
                        buy: _.groupBy(buy_orders.items, (o)=>{ return o.location.id }),
                        sell: _.groupBy(sell_orders.items, (o)=>{ return o.location.id })
                    }

                    return bluebird.each(trx("station_order_stats").where({ region_id: region_id, type_id: type_id }), function(last_stats) {
                        ...
                        buyOrders = _.reject(buyOrders, (o) => { return (o.price < (maxBuyPrice/10)) || (o.minVolume > o.volume) })
                        ...
                        function priceAfterQuantity(target, orders) {
                            const acc = _.reduce(orders, (acc, o) => {
                                ...
                                return acc
                            }, { units: 0, order: { price: null } })

                            return acc.order.price
                        }

                        var buy_price_wavg = (_.sum(_.map(buyOrders, (o) => { return o.price * o.volume })) / buyUnits) || null
                        ...
                        var sell_price_wavg = (_.sum(_.map(sellOrders, (o) => { return o.price * o.volume })) / sellUnits) || null
                        ...
                        _.each([ 'buy', 'sell' ], (n)=>{
                            ...
                            local_change_stats[n+'_price_wavg_sold'] = (_.sum(_.map(sold, (o) => { return o.price * o.units })) / _.sum(_.map(sold, 'units'))) || null
                        ...
                        return bluebird.all([
                            ...
                            // Provide the region_id so that the request can use the index
                            trx("station_order_stats").where({ type_id: type_id, station_id: station_id, region_id: region_id }).update({
                ...
                return trx("market_polling").
                where({ type_id: type_id, region_id: region_id }).
